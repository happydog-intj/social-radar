...
export function generateAppStoreReport(
  reviews: AnalyzedAppStoreReview[],
  date: string,
  language: 'en' | 'zh',
  appInfo: { name: string; averageRating: number; totalReviews: number; currentVersion: string }
): string {
...
function buildDigest(
  ...
  const reviewsByRegion: { [region: string]: AnalyzedAppStoreReview[] } = {};
...
function generateEnglishReport(digest: AppStoreDigest): string {
  const { summary, reviewsByRegion, appInfo } = digest;

  let markdown = `# SHEIN App Store Reviews - ${digest.date}\n\n`;
  ...
  markdown += `- **Period**: Latest reviews from App Store\n\n`;
...
function generateChineseReport(digest: AppStoreDigest): string {
  const { summary, reviewsByRegion, appInfo } = digest;

  let markdown = `# SHEIN App Store 评论分析 - ${digest.date}\n\n`;
